...
You begin at the first position,and winbygettingtothelastposition. Forexample,letA =(3,3,1,0,2,0,1} represent the board game, i.e., 
...
def epi_sol(A: List[int]) -> bool:
    ...
    while(i <= furthest_so_far and furthest_so_far <len(A)):
        print(i, " ", furthest_so_far)
        furthest_so_far = max(furthest_so_far, i + A[i])
        i += 1
    return furthest_so_far >= len(A)
